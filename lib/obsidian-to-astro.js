...
export async function getNote(fileName) {
  ...
  config.sectionsToPull.forEach((sectionName) => {
    const sectionContent = splitContent.find((section) =>
      section.startsWith(sectionName),
    );

    if (sectionContent) {
      const regex = new RegExp(`${sectionName}[ ]*\n`);
      noteSections[sectionName] = sectionContent.replace(regex, "");
    }
  });
...
export function mergeReflexNoteContent(notesObject) {
  ...
  sortMergedContent(reducedWeeks);

  // make our sections into one long string
  let returnString = "\n# Generated\n";
  for (const section in reducedWeeks) {
    const sectionContent = reducedWeeks[section]
      .filter((n) => n != section)
      .sort()
      .join("\n");
...
/**
 * Save a note object to disk, append mode
 *
 * @param {object} note   Our note, expressed as an object
 */
export function appendNote(note) {
  ...
  console.log(`Appending ${note.fileName}...`);
  note = processNote(note);
...
/**
 * Take the merged content and sort it into our notes. this is wehre we deal
 * with the child list, etc
 *
 * @param {*} content
 * @returns
 */
function sortMergedContent(content) {
  ...
  for (const section in content) {
    ...
    const singleBullets = splitContent.filter(
      (bullet) => !bullet[0].startsWith("- "),
    );
    const doubleBullets = splitContent.filter((bullet) =>
      bullet[0].startsWith("- "),
    );

    const reducedDoubleBullets = doubleBullets
      .reduce((prev, curr) => {
        // first, we consolidate our 2d array so one one of each item in index 0 exists
        if (prev.flat().includes(curr[0])) {
          // if the first index of the array exists
      ...
      .map((bulletsArray) => {
        ...
        const sortedBulletsString = bulletsArray
          .slice(1)
          .reduce((prev, curr) => {
            // make sure we're not sending empy bullets
            if (0 === curr.length) {
              return prev;
            }
