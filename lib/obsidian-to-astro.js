...
export function mergeReflexNoteContent(notesObject) {
  ...
  let returnString = '# Generated\n';
  for (const section in reducedWeeks) {
    const sectionContent = reducedWeeks[section].filter(n => n != section).sort().join('\n');
...
function sortMergedContent(content) {
  ...
  for (const section in content) {
    ...
    const singleBullets = splitContent.filter(bullet => !bullet[0].startsWith('- '))
    const doubleBullets = splitContent.filter(bullet => bullet[0].startsWith('- '))
    ...
    const reducedDoubleBullets = doubleBullets.reduce((prev, curr) => {
      ...
      const sortedBulletsString = bulletsArray.slice(1).reduce((prev, curr) => {
