...
function sortNoteSection(section) {
  ...
  if (section.includes('\n##')) {
    const sortedSection = sortNoteSections( 
      section.split(/[\n]?##/)
    )
    return sortedSection.sort().join('\n##')
  }
...
export function mergeReflexNoteContent(notesObject) {
  ...
  for (const section in reducedWeeks) {
    console.log(section, reducedWeeks[section])
    const sectionContent = reducedWeeks[section].filter(n => n != section).join('\n');
